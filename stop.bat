...
for /f "tokens=5" %%a in ('netstat -aon 2^>nul ^| findstr ":3101 "`nfor /f "tokens=5" %%a in ('netstat -aon 2^>nul ^| findstr ":3102 " ^| findstr LISTENING') do taskkill /PID %%a /F >nul 2>&1 ^| findstr LISTENING') do taskkill /PID %%a /F >nul 2>&1
